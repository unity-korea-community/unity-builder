...
namespace Unity_CMD
{
    ...
    public partial class CMDBuilder
    {
        ...
        [MenuItem(const_prefix_EditorContextMenu + "Build Test - IOS")]
        // ReSharper disable once UnusedMember.Global
        public static void BuildTest_IOS()
        {
            BuildConfig config = new BuildConfig();
            BuildTargetGroup buildTargetGroup = GetBuildTargetGroup(BuildTarget.iOS);
            config.definesymbol = PlayerSettings.GetScriptingDefineSymbolsForGroup(buildTargetGroup);

            Build(config, config.buildoutputFolder_absolutepath, config.filename, BuildTarget.iOS);
        }

        [PostProcessBuild(const_PostBuildCallbackOrder)]
        public static void OnPostProcessBuild(BuildTarget buildTarget, string path)
        {
            Debug.Log($"{const_prefix_ForLog} {nameof(OnPostProcessBuild)} - BuildTarget : {buildTarget} strPath : {path}");
            if (buildTarget != BuildTarget.iOS)
                return;

            Init_XCodeProject(path);
            Setup_XCodePlist(path);
        }
        ...
        private static void Init_XCodeProject(string XCodeProjectPath)
        {
#if UNITY_IOS
...
#else
            Debug.Log($"{const_prefix_ForLog} {nameof(Init_XCodeProject)} - Define Symbol is Not IOS");
#endif       
        }
        ...
        private static void CopyFile_Asset_To_XCode(string strXCodeProjectPath, string strFilePath)
        {
            ...
            {
                Debug.Log($"{const_prefix_ForLog} Copy File Error FileName : \"{strFileName}\" // \"{strFilePath_Origin}\" to \"{strFilePath_Dest}\"" + e);
            }

            Debug.Log($"{const_prefix_ForLog} Copy File FileName : \"{strFileName}\" // \"{strFilePath_Origin}\" to \"{strFilePath_Dest}\"");
        }
...
#else
            Debug.Log($"{const_prefix_ForLog} {nameof(Setup_XCodePlist)} - Not Define Symbol is Not IOS");
#endif
        }
